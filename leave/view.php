...
if (isset($_GET['rej'])) {
	...
	if (empty($vra)) {
		...
		if ($u_cot49 > 0) {
			$q_cot_33 = mysqli_fetch_array($u_mysql49);
			if ($date <= $q_cot_33['edate']) {
				echo "<script>document.location = '?m=NOK';</script>";
			} else {
				$isql2 = mysqli_query($db_com, "UPDATE `application` SET `status1`='3', `rejoin`='$date', `mby`='$by' WHERE `lacode`='$vra' LIMIT 1") or die(mysqli_error());
	...
	<div class="row">
		...
		<div class="col-md-10">
			<div class="content-box-header panel-heading">
				<div class="panel-title">ছুটির আবেদনের তালিকা</div>
			</div>
			<div class="content-box-large box-with-header">
				...
				<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
					...
					<tbody>
						<?php
						$sl = 0;
						$q_mysql3 = mysqli_query($db_com, "SELECT * FROM `application` ORDER BY `Id` DESC") or die(mysqli_error());
						...
						if ($q_cot3 > 0) {
							while ($q_cot_row3 = mysqli_fetch_array($q_mysql3)) {
								...
								if ($q_cot_row3['status1'] == '0') {
									...
									$eye = "";
									if ($date > $q_cot_row3['edate']) {
										$rejoin = "<a href='?rej=$q_cot_row3[lacode]' title='Re-Join Now' onclick='return confirmJ(this);'><i class='glyphicon glyphicon-random'></i>";
									} else {
										$rejoin = "";
									}
								} else {
								...
								if ($q_cot_row32['emp_deg'] == 10 or $q_cot_row32['emp_deg'] == 11 or $q_cot_row32['emp_deg'] == 12) {
								    if($q_cot_row34['lid'] == 1){
									    ...
									    $print = "<a href='app-print4.php?aptcc=$q_cot_row3[lacode]' target='_blank' title='App Print' style='color:black;'><i class='glyphicon glyphicon-print'></i></a>";
									}
									...
									if($q_cot_row34['lid'] == 1 or $q_cot_row34['lid'] == 6){
									    $print = "<a href='app-print3.php?aptcc=$q_cot_row3[lacode]' target='_blank' title='App Print' style='color:black;'><i class='glyphicon glyphicon-print'></i></a>";
									}else{
									    $print = "<a href='app-print5.php?aptcc=$q_cot_row3[lacode]' target='_blank' title='App Print' style='color:black;'><i class='glyphicon glyphicon-print'></i></a>";
									}
									$acappdata = "";
								}
