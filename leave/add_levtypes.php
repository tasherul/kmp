...
if (isset($_POST['OK']) == 'Apply' && !empty($_POST['lname'])) {
    ...
    $isql = mysqli_query($db_com, "INSERT INTO `leavetype`(`lcode`, `lname`, `sortname`, `pub`) VALUES ('$scodeee', '$_POST[lname]', '$_POST[sortname]', '1')");
    echo "<script>document.location = '?m=OK';</script>";
}

if (isset($_POST['EOK']) == 'Change' && !empty($_POST['lname']) && !empty($_POST['lcode'])) {
    $isql2 = mysqli_query($db_com, "UPDATE `leavetype` SET `lname`='$_POST[lname]', `sortname`='$_POST[sortname]' WHERE `lcode`='$_POST[lcode]'");
    echo "<script>document.location = '?m=OK';</script>";
}

if (isset($_GET['pub']) && !empty($_GET['pub'])) {
    $isql3 = mysqli_query($db_com, "UPDATE `leavetype` SET `pub`='1' WHERE `lcode`='$_GET[pub]'");
    echo "<script>document.location = '?m=OK';</script>";
}
if (isset($_GET['upub']) && !empty($_GET['upub'])) {
    $isql4 = mysqli_query($db_com, "UPDATE `leavetype` SET `pub`='0' WHERE `lcode`='$_GET[upub]'");
    echo "<script>document.location = '?m=OK';</script>";
}

if (isset($_GET['del']) && !empty($_GET['del'])) {
    $isql4 = mysqli_query($db_com, "DELETE FROM `leavetype` WHERE `lcode`='$_GET[del]'");
    echo "<script>document.location = '?m=OK';</script>";
}
    ...
    <div class="row">
        ...
        <div class="col-md-10">
            <div class="content-box-header panel-heading">
                <div class="panel-title">ছুটির প্রকারভেদ/ধরন</div>
            </div>
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="row">
                        <?php if (isset($_GET['edi']) && !empty($_GET['edi'])) {
                            $q_mysql43 = mysqli_query($db_com, "SELECT * FROM `leavetype` WHERE `lcode`='$_GET[edi]' limit 1") or die(mysqli_error());
                                    ...
                                    <div class="col-md-4"><br /><br /><br />
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label for="h-input">ছুটির প্রকারভেদ/ধরন</label>
                                                <div class="input-group">
                                                    <input type="text" name="lname" value="<?php echo $q_cot_row43['lname']; ?>" class="form-control" required>
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label for="h-input">Short Name</label>
                                                <div class="input-group">
                                                    <input type="text" name="sortname" value="<?php echo $q_cot_row43['sortname']; ?>" class="form-control" required>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div class="col-md-8">
                            <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
                                ...
                                <tbody>
                                    ...
                                    $q_mysql3 = mysqli_query($db_com, "SELECT * FROM `leavetype` ORDER BY `lid` DESC") or die(mysqli_error());
                                    ...
                                    if ($q_cot3 > 0) {
                                        while ($q_cot_row3 = mysqli_fetch_array($q_mysql3)) {
											...
											<tr class='gradeA'>
												...
												<td>$q_cot_row3[sortname] - $q_cot_row3[lname]</td>
                                                <td>$pubdata</td>															
												<td>
